...
IP = "rtsp://192.168.12.39:8554/unicast"

pipeline = (
    "rtspsrc location="
    + IP
    + " latency=0 buffer-mode=auto ! decodebin ! videoconvert ! appsink max-buffers=1 drop=True"
)
...
linear_pid = PID(Kp=25, Ki=0.0, Kd=0.1, setpoint=0.25)  # Target area ratio is 60%
lateral_pid = PID(Kp=2, Ki=0.0, Kd=0.2)

linear_pid_rectangle = PID(
    Kp=20, Ki=0.0, Kd=0.1, setpoint=0.3
    ...
    while not rospy.is_shutdown():
        ...
        if target_color:
            ...
            if best_detection:
                ...
                rospy.loginfo(area_ratio)
            elif done == 1:
                
                if not box:
                    rospy.loginfo("Picking Up The Ball")
                    twist = Twist()
                    twist.linear.x = 0.0
                    twist.angular.z = 0.0
                    cmd_vel_pub.publish(twist)
                    ...
                    rospy.loginfo("Searching For The Box")
                    ...
                    for contour in contours:
                        if cv2.contourArea(contour) > 100:
                            ...
                            if abs(error_x_rectangle) > 0.05 * frame.shape[1]:
                                twist.angular.z = lateral_speed_rectangle

                            if (area_ratio_rectangle > 0.4) and (
                                abs(error_x_rectangle) < 0.05 * frame.shape[1]
                            ):
                                twist.linear.x = 0.0
                                twist.angular.z = 0.0
                                reached_box = 1
                ...
                twist = Twist()
                twist.angular.z = 0.8  # Adjust the rotation speed as needed
                cmd_vel_pub.publish(twist)
